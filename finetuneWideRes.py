...
# Datasets
parser.add_argument('-m', '--model_path', default='checkpoint/cifar10/wideResnet_SDN_w4/SGD/bs128_lr0.1_wd0.0005StepLR_60_0.1/seed_13/finetuneResout0/wide10/addIc_b1b2b3ResOut', type=str)
...
def load_singleWideRes26SDNmodel(model_path):
    ...
    model = resnetSDN(num_classes=10, interOuts=args.interOuts, wideFator=wideFator)
...
def trainOnePhase(model, train_loader, test_loader, criterion, outputIndex):
    ...
    checkpoint = f'finetune/cifar10/WideResnet26/finetuneCheckpoint/seed{args.seed}/wide{wideFator}/{modelOuts}/train{epochs}epochs_lr_{args.lr}'
...
def train(model, trainloader, testloader, criterion):
    ...
    #     if outputIndex != 1:
        ...
        model_path = f'finetune/cifar10/WideResnet26/finetuneCheckpoint/seed{args.seed}/wide{wideFator}/{modelOuts}/train{epochs}epochs_lr_{args.lr}/outputIndex_{outputIndex}_bestModel'
...
def load_singleWideRes26SDNFinetuneModel(model_path):
    # model = WideResNet(interOuts=args.interOuts)
    model = resnetSDN(interOuts=args.interOuts, wideFator=wideFator)
